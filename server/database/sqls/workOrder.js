// 작업지시서 관련 핵심 쿼리문 모음 (수정됨)

// 1. 사원 검색 (모달용)
const searchEmployees = {
  query: `SELECT employee_id, emp_name, dept_name, position
          FROM employees 
          WHERE (? = '' OR emp_name LIKE CONCAT('%', ?, '%') 
...
const searchProducts = {
  query: `SELECT product_code, product_name, product_unit, product_stand, product_pay
          FROM product 
...
const saveWorkOrder = {
  query: `INSERT INTO work_order_master (
...
const deleteWorkOrderProducts = {
  query: `DELETE FROM work_order_product WHERE work_order_no = ?;`
};

const insertWorkOrderProduct = {
